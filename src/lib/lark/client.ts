...
export async function exchangeCodeForToken(
    ...
    if (data.code !== 0) {
        throw new LarkError(
            data.code,
            data.msg || "Failed to exchange code for token",
        );
    }
...
async function getAppAccessToken(): Promise<string> {
    ...
    if (data.code !== 0) {
        throw new LarkError(
            data.code,
            data.msg || "Failed to get app access token",
        );
    }
...
async function makeRequest<T>(
    ...
    console.log("makeRequest | req URL", `${LARK_BASE_URL}${endpoint}`);
    console.log(
    ...
    const data = await response.json();
    console.log("makeRequest | data", JSON.stringify(data, null, 2));
...
export async function createDocument(
    ...
    return makeRequest<LarkCreateDocumentResponse>(
...
export async function createBlocks(
    ...
    console.log(
        "createBlocks | endpoint",
        `/open-apis/docx/v1/documents/${documentId}/blocks/${parentBlockId}/children`,
    );
    console.log("createBlocks | body", JSON.stringify(body, null, 2));
...
export async function updateBlock(
    documentId: string,
    blockId: string,
    block: Partial<LarkBlock>,
): Promise<LarkCreateBlockResponse> {
    console.log("updateBlock | block", JSON.stringify(block));
    let res = makeRequest<LarkCreateBlockResponse>(
    ...
    console.log("updateBlock | res", JSON.stringify(res));
    return res;
}

/**
 * Upload media file to a DocX document
 * Uses the Drive API with docx_image parent type
 */
export async function uploadDocxMedia(
    documentId: string,
    fileName: string,
    fileData: Uint8Array,
    mimeType: string,
    ...
    console.log("uploadDocxMedia | responseJson", JSON.stringify(responseJson));

    const data: LarkUploadMediaResponse = responseJson;
