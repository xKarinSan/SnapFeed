...
async function startNextServer() {
    ...
    nextServer = (0, child_process_1.spawn)(process.execPath.replace("Electron", "node").replace(/Electron\.app.*/, ""), [serverPath], {
        env: {
            ...process.env,
            PORT: String(PORT),
            HOSTNAME: "localhost",
...
async function runMigrations() {
    ...
    return new Promise((resolve) => {
        ...
        const migration = (0, child_process_1.spawn)("node", [prismaPath, "migrate", "deploy", "--schema", schemaPath], {
            env: process.env,
            stdio: "inherit",
        });
...
ipcMain.handle("capture-screen", async (_event, rect) => {
    try {
        if (!mainWindow) {
